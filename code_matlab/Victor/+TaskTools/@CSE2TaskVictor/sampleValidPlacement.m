...
function q = sampleValidPlacement(self, t, varargin)
%   required:
%       t: task progress
%   opts: 
...
%       show_IRM
    ...
    th = pi*th/180;% this is given from args
    ...
    irm_bl = reshape(bl213, self.irm.dim.bl, [])';
    mm = repmat(mask', size(irm_bl,1), 1);
    irm_bl(:, 6:end) = irm_bl(:,6:end)&mm;
    irm_bl(:, 1) = sum(irm_bl(:,6:end)&mm, 2); 
    ...
    if show_IRM
        ...
        if show_method==1
            ...
            for i_H=1:size(unique_H,1)
                ...
                quiver(X,Y,U,V,0,'color',base_color(find(H==h,1),:),'LineWidth',3);   
                hold on
            end
        else
    ...
    % sample n placement
    q = arrayfun(@(~) TaskTools.Q2Pi([-1 -1 -1 -1]), 1:n)';
    for i=1:n
        timeout = 500;
        ...
        while ~valid
            ...
            q_temp = TaskTools.Q2Pi([bl t]);
            valid = self.isValid(q_temp);
            timeout = timeout - 1;
            if timeout <=0
               disp("Timeout! Cannot find placement for this t="+t);
               q = [];
               return;
            end
        end
