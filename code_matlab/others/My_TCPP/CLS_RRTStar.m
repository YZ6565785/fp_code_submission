classdef CLS_RRTStar
    ...
    methods
        ...
        % \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
            ...
            while ~Reached_Goal && ite <= max_iter
                % Initialize:
                NN_val      = Inf;
                pt_new      = node_SE2([]);
                pt_new.cost = Inf;
                ...
                [isReached, isInserted] = CLS_KDTree.insert(pt_new, nodes(1));
                ...
                if this.IsDEBUG
                    ...
                    [progress_plot, Robot_plot, latest_IRM_plot] = this.Plot_state(min_pt, pt_new, s_max);
                    hold on
                    assert(pt_new.parent ~= 0);
                end
            end
            
            q_end	= node_SE2.copy(pt_new);
            path	= q_end;
            ...
            if this.IsDEBUG
                for idx = length(path):-1:2
                    line([path(idx-1).pose(1), path(idx).pose(1)], [path(idx-1).pose(2), path(idx).pose(2)], [0,0], 'Color', '#E1701A', 'LineWidth', 4);
                end
