...
int main(int argc, char **argv)
{
  ...
  std::string sample_filename = path + "sample_poses_100.csv";
  ...
  const int n_poses = sample_poses.size();
  ...
  std::string file;
  n.getParam("create_reachability_map/output", file);
  if (file == "default.h5")
  {
    file = str(boost::format("%s_r%d_reachability_%dposes.h5") % k.getRobotName() % resolution % n_poses);
  }
  ...
  while (ros::ok())
  {
    ...
    pose_col.reserve(new_data.size() * n_poses);
    ...
    for (MultiVector::iterator it = pose_col.begin(); it != pose_col.end(); ++it)
    {
      ...
      int solns;
      // const std::vector<double> pose = it->first;
    ...
    hdf5_dataset::Hdf5Dataset h5(filename); // didn't use this data structure
    ...
    ROS_INFO("*RM Completed");


    ROS_INFO("Writing Boolean Indices to file");
    std::string file_bl = "bl_" + file.substr(0, file.find(".h5")) + ".txt";
    std::string filename_bl = path + file_bl;
    ROS_INFO_STREAM("Will save the boolean indices to " << filename_bl);
    ...
    output_file.close();
    ROS_INFO("*Writing Boolean Indices Completed!");
